...
public class DashboardScreen extends JFrame {
    ...
    public DashboardScreen(BankSystem bankSystem, User user) {
    ...
    private void initializeUI() {
        ...
        infoCard.setPreferredSize(new Dimension(500, 180));
        ...
        accountLabel = new JLabel("Account Numb er: " + user.getAccountNumber());
        ...
        // Button actions with debug dialogs
        depositButton.addActionListener(e -> {
            try {
                new DepositScreen(bankSystem, user).setVisible(true);
                dispose();
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, "Error opening DepositScreen: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            }
        });

        withdrawButton.addActionListener(e -> {
            try {
                new WithdrawScreen(bankSystem, user).setVisible(true);
                dispose();
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, "Error opening WithdrawScreen: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            }
        });

        transferButton.addActionListener(e -> {
            try {
                new TransferScreen(bankSystem, user).setVisible(true);
                dispose();
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, "Error opening TransferScreen: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            }
        });

        loanButton.addActionListener(e -> {
            try {
                new LoanScreen(bankSystem, user).setVisible(true);
                dispose();
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, "Error opening LoanScreen: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            }
        });

        historyButton.addActionListener(e -> {
            try {
                new TransactionHistoryScreen(bankSystem, user).setVisible(true);
                dispose();
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, "Error opening TransactionHistoryScreen: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            }
        });

        profileButton.addActionListener(e -> {
            try {
                new ProfileScreen(bankSystem, user).setVisible(true);
                dispose();
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, "Error opening ProfileScreen: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            }
        });

        changePinButton.addActionListener(e -> {
            try {
                new ChangePinScreen(bankSystem, user).setVisible(true);
                dispose();
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, "Error opening ChangePinScreen: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            }
        });
        ...
        logoutButton.addActionListener(e -> {
            try {
                ...
                JOptionPane.showMessageDialog(this, "Error during logout: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            }
        });
    ...
    private void refreshUserData() {
        try {
            ...
            User updatedUser = bankSystem.getAllUsers().stream()
                    .filter(u -> u.getAccountNumber().equals(user.getAccountNumber()))
                    .findFirst()
                    .orElse(null);
            if (updatedUser != null) {
                this.user = updatedUser;
                accountLabel.setText("Account Number: " + user.getAccountNumber());
                balanceLabel.setText("Current Balance: $" + String.format("%.2f", user.getBalance()));
                loanLabel.setText("Loan Amount: $" + String.format("%.2f", user.getLoanAmount()));
            } else {
                JOptionPane.showMessageDialog(this, "Account not found after refresh.", "Error", JOptionPane.ERROR_MESSAGE);
            }
        } catch (Exception e) {
            System.err.println("Error refreshing account data: " + e.getMessage());
            JOptionPane.showMessageDialog(this, "Failed to refresh account data: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private JPanel createActionCard(String title, JButton button) {
        ...
        card.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(new Color(200, 200, 200)),
                BorderFactory.createEmptyBorder(15, 15, 15, 15)));
        card.setPreferredSize(new Dimension(180, 140)); // Increased from 130px
